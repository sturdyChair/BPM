...
void CFrustum::Update()
{
	...
	{
		vPoints[i] = XMVector3TransformCoord(XMLoadFloat3(&m_vPoints[i]), m_pGameInstance->Get_Transform_Matrix(CPipeLine::TS_PROJ_INV));
		vPoints[i] = XMVector3TransformCoord(vPoints[i], m_pGameInstance->Get_Transform_Matrix(CPipeLine::TS_VIEW_INV));
...
{
	__super::Free();
	if(m_pGameInstance->Get_RefCount() != 0)
