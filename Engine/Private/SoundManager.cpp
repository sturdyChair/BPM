...
void CSoundManager::Update()
{
    if(m_pListener)
    {
        CTransform* pTransform = m_pListener->Get_Transform();
...
HRESULT CSoundManager::Add_Sound(SOUND_CHANNEL eChannel ,const wstring& strSoundTag, const wstring& strSoundPath)
{
    ...
    CSound* pSound = CSound::Create(m_pFmodSystem, strSoundPath);
    if (!pSound)
    {
        return E_FAIL;
    }
...
HRESULT CSoundManager::Add_Sound_3D(SOUND_CHANNEL eChannel, const wstring& strSoundTag, const wstring& strSoundPath)
{
    ...
    C3D_Sound* pSound = C3D_Sound::Create(m_pFmodSystem, strSoundPath);
    if (!pSound)
    {
        return E_FAIL;
    }
...
{
    CSound* pSound = Find_Sound(eChannel, strSoundTag);
    if (pSound)
    {
        //pSound->Stop();
        pSound->Play(bLoop);
...
{
    C3D_Sound* pSound = Find_Sound_3D(eChannel, strSoundTag);
    if (pSound)
    {
        //pSound->Stop();
...
CSound* CSoundManager::Find_Sound(SOUND_CHANNEL eChannel, const wstring& strSoundTag)
{
...
C3D_Sound* CSoundManager::Find_Sound_3D(SOUND_CHANNEL eChannel, const wstring& strSoundTag)
{
...
void CSoundManager::Set_Listener(CGameObject* pListener)
{
