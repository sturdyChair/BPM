...
HRESULT CCamera::Initialize(void* pArg)
{
	...
	if (pArg)
	{
		...
		m_pGameInstance->Add_Camera(pDesc->strTag, this);
	}
	else
	{
		++iCamID;
		m_pGameInstance->Add_Camera(to_wstring(iCamID), this);
...
{
	m_pGameInstance->Set_Transform(CPipeLine::PIPELINE_STATE::TS_VIEW, m_pTransformCom->Get_WorldMatrix_Inversed());
	m_pGameInstance->Set_Transform(CPipeLine::PIPELINE_STATE::TS_PROJ, m_ProjMatrix);
}
