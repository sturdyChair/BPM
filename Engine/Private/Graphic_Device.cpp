...
HRESULT CGraphic_Device::Initialize(const ENGINE_DESC& EngineDesc, _Inout_ ID3D11Device** ppDevice, _Inout_ ID3D11DeviceContext** ppContext)
{
	...
	ID3D11RenderTargetView*		pRTVs[3] = {
		m_pBackBufferRTV,
		m_RenderTargets[strObjectIdRenderTargetTag]->Get_RTV(),
		m_RenderTargets[strPositionRenderTargetTag]->Get_RTV(),
	};
...
HRESULT CGraphic_Device::Clear_RTVs(_float4 vClearColor)
{
	...
	//m_pDeviceContext->ClearRenderTargetView(m_pAlbedoRTV, (_float*)&vClearColor);
	//m_pDeviceContext->ClearRenderTargetView(m_pNormalRTV, (_float*)&vClearColor);
	//m_pDeviceContext->ClearRenderTargetView(m_pOrmRTV, (_float*)&vClearColor);
	//m_pDeviceContext->ClearRenderTargetView(m_pPositionRTV, (_float*)&vClearColor);
	//vClearColor = _float4{ 0.f,0.f,0.f,0.f };
	//m_pDeviceContext->ClearRenderTargetView(m_pIDRTV, (_float*)&vClearColor);
...
HRESULT CGraphic_Device::Ready_RenderTargets(_uint iWinCX, _uint iWinCY)
{
	...
	Add_RenderTarget(strGlowPowerRenderTargetTag, iWinCX / 4, iWinCY / 4, DXGI_FORMAT_R8G8B8A8_UNORM, _float4{ 0.f,0.f,0.f,0.f });
	Add_RenderTarget(strGlowBlurRenderTargetTag, iWinCX /4, iWinCY /4, DXGI_FORMAT_R8G8B8A8_UNORM, _float4{ 0.f,0.f,0.f,0.f });
	...
	//m_pDevice->CreateRenderTargetView(m_pAlbedoTexture,	  NULL, &m_pAlbedoRTV);
	//m_pDevice->CreateRenderTargetView(m_pNormalTexture,	  NULL, &m_pNormalRTV);
	//m_pDevice->CreateRenderTargetView(m_pOrmTexture,	  NULL, &m_pOrmRTV);
	//m_pDevice->CreateRenderTargetView(m_pPositionTexture, NULL, &m_pPositionRTV);
	//m_pDevice->CreateRenderTargetView(m_pIDTexture,		  NULL, &m_pIDRTV);
	//
	//m_pDevice->CreateShaderResourceView(m_pAlbedoTexture,	NULL, &m_pAlbedoSRV);
	//m_pDevice->CreateShaderResourceView(m_pNormalTexture,	NULL, &m_pNormalSRV);
	//m_pDevice->CreateShaderResourceView(m_pOrmTexture,		NULL, &m_pOrmSRV);
	//m_pDevice->CreateShaderResourceView(m_pPositionTexture, NULL, &m_pPositionSRV);
	//m_pDevice->CreateShaderResourceView(m_pIDTexture,		NULL, &m_pIDSRV);
...
HRESULT CGraphic_Device::Add_RenderTarget(const _wstring& strRenderTargetTag, _uint iWidth, _uint iHeight, DXGI_FORMAT ePixel, const _float4& vClearColor)
{
	...
	CRenderTarget* pRenderTarget = CRenderTarget::Create(m_pDevice, m_pDeviceContext, iWidth, iHeight, ePixel, vClearColor);
...
HRESULT CGraphic_Device::Bind_RT_SRV(const _wstring& strRTTag, CShader* pShader, const _char* pConstantName)
{
...
CRenderTarget* CGraphic_Device::Find_RenderTarget(const _wstring& strRenderTargetTag)
{
...
list<class CRenderTarget*>* CGraphic_Device::Find_MRT(const _wstring& strMRTTag)
{
...
HRESULT CGraphic_Device::Render_Debug(const _wstring& strMRTTag, CShader* pShader, const _char* pConstantName, CVIBuffer_Rect* pVIBuffer)
{
...
void CGraphic_Device::Free()
{
	...
	//Safe_Release(m_pOrmTexture);
	//Safe_Release(m_pPositionTexture);
	//Safe_Release(m_pAlbedoRTV);
	//Safe_Release(m_pNormalRTV);
	//Safe_Release(m_pOrmRTV);
	//Safe_Release(m_pPositionRTV);
	//Safe_Release(m_pAlbedoSRV);
	//Safe_Release(m_pNormalSRV);
	//Safe_Release(m_pOrmSRV);
	//Safe_Release(m_pPositionSRV);
	//
	//Safe_Release(m_pIDTexture);
	//Safe_Release(m_pIDRTV);
	//Safe_Release(m_pIDSRV);
