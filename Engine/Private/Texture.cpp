...
HRESULT CTexture::Initialize_Prototype(const _wstring& strTextureFilePath, _uint iNumTexture)
{
	...
	for (_uint i = 0; i < iNumTexture; ++i)
	{
		...
		{
			if (FAILED(CreateDDSTextureFromFile(m_pDevice, szFilePath, nullptr, &m_SRVs[i])))
			{
...
HRESULT CTexture::Bind_ShaderResource(CShader* pShader, const _char* pConstantName, _uint iTextureIdx)
{
	return pShader->Bind_SRV(pConstantName, m_SRVs[iTextureIdx]);
}

HRESULT CTexture::Bind_ShaderResources(CShader* pShader, const _char* pConstantName, _uint iFromIdx)
{
	return pShader->Bind_SRVs(pConstantName, m_SRVs.data() + iFromIdx, m_iNumTexture - iFromIdx);
}

CTexture* CTexture::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _wstring& strTextureFilePath, _uint iNumTexture)
{
