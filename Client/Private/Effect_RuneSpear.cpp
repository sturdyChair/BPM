...
CEffect_RuneSpear::CEffect_RuneSpear(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) :
	CEffectObject(pDevice, pContext)
{
}

CEffect_RuneSpear::CEffect_RuneSpear(const CEffect_RuneSpear& rhs) :
	CEffectObject(rhs)
{
	m_pPlayerManager = CPlayer_Manager::Get_Instance();
}

HRESULT CEffect_RuneSpear::Initialize(void* Arg)
{
	...
	Add_Component(LEVEL_STATIC, L"Prototype_Component_Model_RuneSpear", L"Com_Model", (CComponent**)&m_pModelCom);

	return S_OK;
}

HRESULT CEffect_RuneSpear::Initialize_Prototype()
{
...
void CEffect_RuneSpear::PriorityTick(_float fTimeDelta)
{
}

void CEffect_RuneSpear::Tick(_float fTimeDelta)
{
...
void CEffect_RuneSpear::LateTick(_float fTimeDelta)
{

	m_pGameInstance->Add_RenderObjects(CRenderer::RENDER_BLEND_NON_LIT, this);
}

HRESULT CEffect_RuneSpear::Render()
{
	...
	for (int i = 0; i < m_pModelCom->Get_NumMesh(); ++i)
	{
		...
		m_pModelShaderCom->Begin(_uint(SHADER_VTXMESH_PASS::Emission));
...
void CEffect_RuneSpear::Push_From_Pool(void* pArg)
{
	m_fLifeTime = 5.f;
...
void CEffect_RuneSpear::Dying()
{

}

void CEffect_RuneSpear::Collision_Enter(CGameObject* pOther, const PxControllerHit& ControllerHit)
{
	if (Is_Dead())
		return;
	if (m_iCollisionGroup == CG_MONSTER_PROJECTILE)
...
CEffect_RuneSpear* CEffect_RuneSpear::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_RuneSpear* pInstance = new CEffect_RuneSpear(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CEffect_RuneSpear"));
...
CGameObject* CEffect_RuneSpear::Clone(void* pArg)
{
	CEffect_RuneSpear* pInstance = new CEffect_RuneSpear(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CEffect_RuneSpear"));
		Safe_Release(pInstance);
	}
