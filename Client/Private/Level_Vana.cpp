...
CLevel_Vana::CLevel_Vana(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CLevel{ pDevice, pContext }
{

}

HRESULT CLevel_Vana::Initialize()
{
	m_iLevelID = LEVEL_VANA;
	...
	CBeat_Manager::Get_Instance()->Queue_Sequence(L"Vana");
	_uint iHel = 1;
	...
	CRoomManager::Get_Instance()->Start(LEVEL_VANA);
	...
	desc.color = { 0.1f,0.1f,0.025f };
	desc.direction = { 0.5f,-0.5f,0.f };
	desc.intensity = 1.f;
	desc.specColor = { 0.1f,0.1f,0.025f };
...
void CLevel_Vana::Tick(_float fTimeDelta)
{
	if (GetFocus())
...
HRESULT CLevel_Vana::Ready_Layer_Camera(const wstring& strLayerTag)
{
	return S_OK;
}

HRESULT CLevel_Vana::Ready_Layer_BackGround(const wstring& strLayerTag)
{

	return S_OK;
}

HRESULT CLevel_Vana::Ready_Layer_Effect(const wstring& strLayerTag)
{
...
HRESULT CLevel_Vana::Ready_Layer_UI(const wstring& strLayerTag)
{
	...
	pUI_Controller->Push_UI(LEVEL_VANA);
...
HRESULT CLevel_Vana::Ready_LandObjects()
{
...
CLevel_Vana* CLevel_Vana::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLevel_Vana* pInstance = new CLevel_Vana(pDevice, pContext);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX(TEXT("Failed to Created : CLevel_Vana"));
