...
_int CEffect_Sharp_Projectile::iLightIdx = 0;

CEffect_Sharp_Projectile::CEffect_Sharp_Projectile(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) :
	CEffectObject(pDevice, pContext)
{
}

CEffect_Sharp_Projectile::CEffect_Sharp_Projectile(const CEffect_Sharp_Projectile& rhs) :
	CEffectObject(rhs)
{
	m_pPlayerManager = CPlayer_Manager::Get_Instance();
}

HRESULT CEffect_Sharp_Projectile::Initialize(void* Arg)
{
	...
	Add_Component(LEVEL_STATIC, L"Prototype_Component_Model_Sharp_Projectile", L"Com_Model", (CComponent**)&m_pModelCom);
...
HRESULT CEffect_Sharp_Projectile::Initialize_Prototype()
{
...
void CEffect_Sharp_Projectile::PriorityTick(_float fTimeDelta)
{
	m_pParticleTail->PriorityTick(fTimeDelta);
	m_pTrail->PriorityTick(fTimeDelta);
}

void CEffect_Sharp_Projectile::Tick(_float fTimeDelta)
{
...
void CEffect_Sharp_Projectile::LateTick(_float fTimeDelta)
{
...
HRESULT CEffect_Sharp_Projectile::Render()
{
...
void CEffect_Sharp_Projectile::Push_From_Pool(void* pArg)
{
	m_fLifeTime = 5.f;
...
void CEffect_Sharp_Projectile::Dying()
{
...
void CEffect_Sharp_Projectile::Collision_Enter(CGameObject* pOther, const PxControllerHit& ControllerHit)
{
	if (Is_Dead())
		return;
	if (m_iCollisionGroup == CG_MONSTER_PROJECTILE)
...
CEffect_Sharp_Projectile* CEffect_Sharp_Projectile::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Sharp_Projectile* pInstance = new CEffect_Sharp_Projectile(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CEffect_Sharp_Projectile"));
...
CGameObject* CEffect_Sharp_Projectile::Clone(void* pArg)
{
	CEffect_Sharp_Projectile* pInstance = new CEffect_Sharp_Projectile(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CEffect_Sharp_Projectile"));
		Safe_Release(pInstance);
	}
...
void CEffect_Sharp_Projectile::Free()
{
	__super::Free();
