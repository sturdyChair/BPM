...
CEffect_GideonSpear::CEffect_GideonSpear(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) :
	CEffectObject(pDevice, pContext)
{
}

CEffect_GideonSpear::CEffect_GideonSpear(const CEffect_GideonSpear& rhs) :
	CEffectObject(rhs)
{
	m_pPlayerManager = CPlayer_Manager::Get_Instance();
}

HRESULT CEffect_GideonSpear::Initialize(void* Arg)
{
	...
	Add_Component(LEVEL_STATIC, L"Prototype_Component_Model_GideonSpear", L"Com_Model", (CComponent**)&m_pModelCom);
	...
	tdesc.strTextureTag = L"Prototype_Component_Texture_ElectricTrail";
...
HRESULT CEffect_GideonSpear::Initialize_Prototype()
{
...
void CEffect_GideonSpear::PriorityTick(_float fTimeDelta)
{
	m_pTrail->PriorityTick(fTimeDelta);
}

void CEffect_GideonSpear::Tick(_float fTimeDelta)
{
...
void CEffect_GideonSpear::LateTick(_float fTimeDelta)
{
...
HRESULT CEffect_GideonSpear::Render()
{
...
void CEffect_GideonSpear::Push_From_Pool(void* pArg)
{
	m_fLifeTime = 5.f;
...
void CEffect_GideonSpear::Dying()
{
	m_pTrail->Dying();
}

void CEffect_GideonSpear::Collision_Enter(CGameObject* pOther, const PxControllerHit& ControllerHit)
{
	if (Is_Dead())
		return;
	if (m_iCollisionGroup == CG_MONSTER_PROJECTILE)
...
CEffect_GideonSpear* CEffect_GideonSpear::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_GideonSpear* pInstance = new CEffect_GideonSpear(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Create : CEffect_GideonSpear"));
...
CGameObject* CEffect_GideonSpear::Clone(void* pArg)
{
	CEffect_GideonSpear* pInstance = new CEffect_GideonSpear(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Clone : CEffect_GideonSpear"));
		Safe_Release(pInstance);
	}
